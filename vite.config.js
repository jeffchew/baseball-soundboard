...
// Get git commit hash or use timestamp as fallback
const getVersion = () => {
  try {
  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        // Cache audio files with CacheFirst strategy for instant playback
        // Falls back to network only if not in cache, with longer timeout for reliability
        runtimeCaching: [
          {
            urlPattern: /\.mp3(\?.*)?$/i,
            handler: 'CacheFirst',
